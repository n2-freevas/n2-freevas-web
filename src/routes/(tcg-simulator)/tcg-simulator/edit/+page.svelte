<script lang="ts">
  ...
  async function onFileSelected(
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      let count = 1;
      let match: RegExpMatchArray | null = images[i].name.match(/\[[0-9]*\]/g);
      if (match) {
...
<style lang="scss">
  ...
  #exdeck {
    display: flex;
    width: calc(
      var(--card-width) * var(--wrap_num) + var(--card-margin) *
        (var(--wrap_num) * 2)
    );
    margin: 0 auto;
    flex-wrap: wrap;
