<script lang="ts">
  ...
  import DeckAndHand from "../_components/deckAndHand.svelte";
  import type { deckCardModel } from "../_components/TCGsimModel";
  import {
    modeStore,
    boardAreaInfoStore,
    handAreaInfoStore,
    ...
    handListStore,
    boardListStore,
  } from "../_components/TCGsimStore";
  import { cardInAnywhere, cardOutAnywhere } from "../_components/util.svelte";
  import StorageComponent from "../../../(pokemon-dmg-cnt)/pokemon-card-game-effector/_component/StorageComponent.svelte";
  import CoinComponent from "../../../(pokemon-dmg-cnt)/pokemon-card-game-effector/_component/CoinComponent.svelte";
  import PokemonUtil from "../_components/pokemonUtil.svelte";
  ...
  function _getHandInfo() {
    const handInfo = document
      .getElementById("handArea")
  ...
  function _getDeckInfo() {
    const deckInfo = document
      .getElementById("deckArea")
      ?.getBoundingClientRect();
    if (!!deckInfo)
      $deckAreaInfoStore = {
        top: deckInfo.top,
  ...
  function _windowChangeHandler() {
    ...
    if (
      window_width < window_width_require ||
      window_height_require > window_height
    ) {
      isPC = false;
    }
  ...
  function cardHandInFromBoard(event: any) {
  ...
  // DecK In Functions
  function cardDeckInFromBoard(event: any) {
    const id = event.detail.id;
    const _t = $boardListStore.filter((card) => card.id == id).pop();
    if (!_t) return;
    ...
    <div class="panel">
      ...
      <div class="device-exp">
        <div class="device-figure">
          ...
          <div class="height">
            {`${window_height_require}px`}<span
              >{` ( ${window_height}px )`}</span
            >
          </div>
          <img
