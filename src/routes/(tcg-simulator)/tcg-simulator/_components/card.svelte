<script lang="ts">
  ...
  import type { OnAreaModel } from "./TCGsimModel";
    ...
    if (moving) {
      ...
      //デッキエリアに突入しているか判定する。
        ...
        if (onArea != "deck") {
          ...
          const to =
            postPosition <=
            ($deckAreaInfoStore.bottom - $deckAreaInfoStore.top) / 2
              ? "top"
              : "bottom";
...
<section
  class="card-id-{id} card-body {moving ? 'movin' : ''} {noGuide
    ? 'noGuide'
    : ''} {onArea != 'board' ? 'noBoard' : ''}"
...
>
  {#if !$movingStore}
    <div class="card-guide {overing ? 'hover' : ''}">
      {#if overing && !moving}
        <button class="left-rotate" on:click={cardLeftRotate} />
        <button class="right-rotate" on:click={cardRightRotate} />
        <button class="flip" on:click={cardFlip} />
      {/if}
    </div>
  {/if}
  <div
    class="card-surface {flippin ? 'flippin' : ''}"
  ...
  {#if moving && !noGuide}
    <section
      class="card-shadow"
      style="--pos_x:{pos_x}px;--pos_y:{pos_y}px;--img:{img_url}"
    >
      {#if flippin}
        <img src={sleeve_url} alt="" />
      {:else}
