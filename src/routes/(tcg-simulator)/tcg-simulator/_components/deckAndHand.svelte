<script context="module" lang="ts">
  import type { deckCardModel } from "./TCGsimModel";
  import FukidashiConponent from "../../../(pokemon-dmg-cnt)/pokemon-card-game-effector/_component/FukidashiConponent.svelte";
...
<script lang="ts">
  ...
  import type { handCardModel } from "./TCGsimModel";
  import { cardInAnywhere, cardOutAnywhere } from "./util.svelte";
  ...
  function handLineUp() {
    ...
    const x_offset =
      ($handAreaInfoStore.left + $handAreaInfoStore.right) / 2 - $cardWidth;
    ...
    for (let i = 0; i < handLen; i++) {
      ...
      if (hand[i]) {
        ...
        const y =
          -1 * Math.sqrt(Math.pow(handLineupGuideRadius, 2) - Math.pow(x, 2));
